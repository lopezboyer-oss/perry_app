...
export default auth((req) => {
  ...
  // Public routes
  const publicRoutes = ['/login', '/api/auth', '/campo', '/cliente-envio', '/api/campo', '/api/cliente-envio', '/api/whatsapp', '/api/cron'];
...
export const config = {
  matcher: ['/((?!_next/static|_next/image|favicon.ico|.*\\.png$|.*\\.jpg$|.*\\.jpeg$|.*\\.svg$|.*\\.ico$|.*\\.mp3$|.*\\.ogg$|.*\\.wav$|.*\\.m4a$|.*\\.opus$|.*\\.webm$|.*\\.html$).*)'],
};
