...
export function PlanDiarioClient() {
  ...
  const handleOpenModal = (compName?: string) => {
    ...
    const existingPlan = plans.find((p) => p.companyName.toUpperCase() === targetComp.toUpperCase());

    if (existingPlan && existingPlan.activities.length > 0) {
    ...
    try {
      const res = await fetch('/api/plan-diario', {
        ...
        body: JSON.stringify({
          ...
          activities: modalActivities.filter((a) => a.title.trim().length > 0),
          personnelStatus: modalPersonnelStatus.filter((ps) => ps.personName.trim().length > 0),
        }),
      });
      ...
      if (res.ok) {
        ...
        alert('Error al guardar el Plan Diario');
      }
    } catch (err) {
      console.error(err);
  ...
  // Calculate Metrics
  const totalActivities = plans.reduce((acc, p) => acc + p.activities.length, 0);
  const totalCrossSupport = plans.reduce(
    (acc, p) => acc + p.activities.filter((a) => a.isCrossSupport || (a.assignedPersonnel || '').toLowerCase().includes('global')).length,
    0
  );
  const totalRestPersonnel = plans.reduce((acc, p) => acc + p.personnelStatus.length, 0);
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 p-12 rounded-3xl border border-slate-800 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1">Haz clic en &quot;Cargar / Editar Plan&quot; para registrar las actividades y personal.</p>
          </div>
          <button
            onClick={() => handleOpenModal()}
            ...
            <div key={plan.id} className="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden shadow-xl">
              ...
              <div className="bg-slate-950 p-5 border-b border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg md:text-xl font-black text-white tracking-tight">{plan.companyName.toUpperCase()}</h2>
                    <span className="text-slate-400 text-xs font-semibold">
                      Plan Diario • {plan.activities.length} Actividades Programadas
                    </span>
                  </div>
                </div>
              ...
              {plan.personnelStatus && plan.personnelStatus.length > 0 && (
                <div className="bg-slate-950/80 p-4 border-t border-slate-800">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {plan.personnelStatus.map((ps, idx) => {
