...
export default async function SummerShutDownPage() {
  ...
  // For tech/contractor plans: fetch ALL activities across all companies in the shutdown range
  ...
  const planDays = allDates.map(dateStr => {
    ...
    return {
      date: dateStr,
      isExtra: false, // Mark all days as permanent for the Summer Shut Down plan
      extraId: null,
      label: dateStr === saturday || dateStr === sunday || dateStr === '2026-07-18' || dateStr === '2026-07-19' ? null : 'Shut Down',
      hasActivities: dayActivities.length > 0,
    };
  });

  // Build weekend label from visible days
  const weekendLabel = 'Summer Shut Down (11 — 19 Jul 2026)';
