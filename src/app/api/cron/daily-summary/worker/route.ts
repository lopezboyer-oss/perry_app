...
// POST /api/cron/daily-summary/worker?secret=CRON_SECRET
// Called by the trigger endpoint. Runs with its own 60s timeout.
// Generates the director summary and sends it via WhatsApp.
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      summaryRes = await fetch(`${appUrl}/api/whatsapp/director-summary`, {
        method: 'POST',
        headers: {
      ...
      if (fetchErr.name === 'AbortError') {
        console.error('[CRON-WORKER] Director summary timeout (50s)');
        return NextResponse.json({ error: 'Director summary timeout (50s)' }, { status: 504 });
      }
      throw fetchErr;
    } finally {
      clearTimeout(fetchTimeout);
    ...
    if (!summaryRes.ok) {
      ...
      console.error('[CRON-WORKER] Director summary API error:', summaryRes.status, errText);
    ...
    if (!summary || !summary.executiveSummary) {
      console.error('[CRON-WORKER] No summary data returned');
    ...
    console.log(`[CRON-WORKER] Summary ${sent ? 'sent successfully' : 'FAILED to send'}. Length: ${fullMessage.length}`);
