...
// ENDPOINT TEMPORAL — Generar nota de voz de Perry con Gemini TTS
// Eliminar después de generar el audio
export async function GET() {
  ...
  try {
    ...
    if (!audioPart) {
      return NextResponse.json({ error: 'No audio in response', raw: JSON.stringify(json).substring(0, 300) }, { status: 500 });
    }

    // Gemini TTS returns raw PCM (signed 16-bit little-endian, 24000 Hz, mono)
    // Wrap in WAV header for playability
